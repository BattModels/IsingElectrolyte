...
def energetics(
    ...
    # Symmetrizing here, at the assembly point rather than inside the term
    # functions, also covers any custom injected J_sol_sol_func / J_anion_anion_func
    # for free — the term functions see property values, never species indices,
    # so a rule that depends on group rank cannot be expressed there.
    if pair_symmetry == "group":
        n_sol   = sol_props["dn"].shape[0]
        n_anion = anion_props["dn"].shape[0]
        grp = _resolve_groups(groups, n_sol + n_anion)
    ...
    # Assemble the block matrix — jnp.block builds the 2x2 layout in one XLA op.
