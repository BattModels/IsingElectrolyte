...
def old_J_anion_anion(props_i, props_j, p):
...
def _energetics_generic(params, sol, anion, groups, pair_symmetry="group"):
    ...
    ``energetics`` takes species-property dicts (the descriptor-extensible API),
    so the tuples above are packed into the {"dn", "an", "x", "v"} layout that
    ``_species_props_to_arrays`` produces for real callers.
    """
    sol_props = {
        k: jnp.array([s[i] for s in sol])
        for i, k in enumerate(("dn", "an", "x", "v"))
    }
    anion_props = {
        "dn": jnp.array([anion[0]]),
        "x":  jnp.array([anion[1]]),
        "v":  jnp.array([anion[2]]),
    }
    return energetics(
        jnp.ones(len(sol) + 1) / (len(sol) + 1), dict(params),
        sol_props, anion_props, 1.86,
